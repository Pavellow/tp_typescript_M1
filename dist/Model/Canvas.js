...
class Canvas {
    ...
    _width;
    _height;
    constructor(widthParam, heightParam, joueurParam, asteroidesParam, intervalParam, scoreParam) {
        this._width = widthParam;
        this._height = heightParam;
        this._canvas = document.querySelector("#jeu");
    ...
    generateAsteroides() {
        ...
        let aste = new Asteroid(10, 150, 10, 1, coordX, coordY, this.genererandom(0, 6), this.genererandom(0, 6));
    ...
    drawProjectile(projectile) {
        ...
        ctx.arc(this._joueur.getPosX(), this._joueur.getPosY(), radius, 0, Math.PI * 2, true); // Dessin du projectile centré sur sa position actuelle
    ...
    static asteroidMort(asteroids, asteroidMort) {
        return asteroids.filter((asteroid) => asteroid !== asteroidMort);
    }
    static checkImpactAsteroid(canvas, projectiles, asteroids) {
        projectiles.forEach((projectile) => {
            asteroids.forEach((asteroid) => {
                const distance = Math.sqrt(Math.pow(projectile.getPosX() - asteroid.getPoxX(), 2) +
    ...
    static ameliorationJoueur(canvas, score, joueur, munition) {
        if (score >= 500) {
